/* ==========================================================================
   3D Character Controller - NaDream Chibi Avatar with Cool Badminton Sneakers
   ========================================================================== */
...
class CharacterController {
  ...
  createCoolSneakers() {
    ...
    // 1. Thick Cushioned White Sole
    const soleGeo = new THREE.BoxGeometry(0.26, 0.08, 0.42);
    const soleMat = new THREE.MeshToonMaterial({ color: 0xffffff });
    const soleMesh = new THREE.Mesh(soleGeo, soleMat);
    soleMesh.position.set(0, -0.48, 0.06);
    sneakerGroup.add(soleMesh);

    // 2. Cyan Neon Bottom Trim
    const accentGeo = new THREE.BoxGeometry(0.27, 0.03, 0.43);
    const accentMat = new THREE.MeshBasicMaterial({ color: 0x00f5d4 });
    const accentMesh = new THREE.Mesh(accentGeo, accentMat);
    accentMesh.position.set(0, -0.51, 0.06);
    sneakerGroup.add(accentMesh);

    // 3. Sneaker Upper Body (Sleek Dark Blue / Navy)
    const upperGeo = new THREE.BoxGeometry(0.24, 0.14, 0.38);
    const upperMat = new THREE.MeshToonMaterial({ color: 0x0f172a });
    const upperMesh = new THREE.Mesh(upperGeo, upperMat);
    upperMesh.position.set(0, -0.41, 0.06);
    sneakerGroup.add(upperMesh);

    // 4. White Front Toe Cap
    const toeGeo = new THREE.BoxGeometry(0.24, 0.12, 0.16);
    const toeMesh = new THREE.Mesh(toeGeo, soleMat);
    toeMesh.position.set(0, -0.42, 0.17);
    sneakerGroup.add(toeMesh);

    // 5. Gold Pro Side Stripes
    const stripeGeo = new THREE.BoxGeometry(0.26, 0.04, 0.22);
    const stripeMat = new THREE.MeshBasicMaterial({ color: 0xffd700 });
    const stripeMesh = new THREE.Mesh(stripeGeo, stripeMat);
    stripeMesh.position.set(0, -0.42, 0.05);
    sneakerGroup.add(stripeMesh);
  ...
  initChibiMesh() {
    ...
    // Left Cool Sneaker
    const leftSneaker = this.createCoolSneakers();
    leftLegGroup.add(leftSneaker);
    ...
    // Right Cool Sneaker
    const rightSneaker = this.createCoolSneakers();
    rightLegGroup.add(rightSneaker);
    ...
    // Left Arm (Positioned at Shoulder height y=1.25)
    const leftArmGroup = new THREE.Group();
    leftArmGroup.position.set(-0.44, 1.25, 0);
    ...
    // Right Arm (Positioned at Shoulder height y=1.25)
    const rightArmGroup = new THREE.Group();
    rightArmGroup.position.set(0.44, 1.25, 0);
